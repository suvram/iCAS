...
namespace Micro.WebApplication.MicroERP.ADMIN
{
	...
	public partial class MenuItems : BasePage
	{
		...
		{
            //if (!ddl_MenuHead.SelectedValue.ToString().Equals(string.Empty))
            //{
            ...
            gview_Menu.Columns[1].Visible = false;
                ...
                {
                    //int MenuID = int.Parse(ddl_MenuHead.SelectedValue);

                    //var TheMenuList = from m in PageVariables.MenuList
                    //                  where m.ParentWebMenuID == MenuID
                    //                  select m;
